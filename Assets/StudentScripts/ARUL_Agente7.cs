...
public class ARLU_Agente7 : ISSR_Agent
{
    ...
    { // SF “State Function”
        ...
        { // Según el evento
            ...
            case ISSREventType.onDestArrived:
                Debug.LogFormat("on dest arrived");
    ...
    { // SF “State Function”
        ...
        { // Según el evento
            case ISSREventType.onGObjectCollision:
                ...
                break;
            case ISSREventType.onCollision:
                if ((last_state == ISSRState.GoingToGoalWithSmallStone) || (last_state == ISSRState.Scouting) && (iMovingStonesInMyTeam() != 0)) { next_state = ISSRState.WaitforNoStonesMoving; }
                else { next_state = ProcessCollision(); }
                break;
            case ISSREventType.onDestArrived:
    ...
    ISSRState GetBStone(ISSR_Object stone)
    {
        ...
        else
        {
            if (Vector3.Distance(oiLastLocation(Myself), oiLastLocation(stone)) > iSensingRange())
            {
                if (Valid_Big_Stones.Contains(stone))//
                    acGotoLocation(oiLastLocation(stone));
    ...
    { // SF “State Function”
        ...
        { // Según el evento
            case ISSREventType.onTickElapsed:
                Debug.LogFormat("{0}: {1} agentes agarrando la piedra", Myself.Name, GrippingAgentsMyTeam(focus_object));
            ...
            case ISSREventType.onAnotherAgentGripped:
                ...
                if (GrippingAgentsMyTeam(focus_object) >= 2)
                {
                    Debug.LogFormat("{0} agentes agarrando la piedra", GrippingAgentsMyTeam(focus_object));
                    focus_object.TimeStamp = Time.time;
                    BStoneIsAvailable(focus_object, false);
                    next_state = ISSRState.WaitforNoStonesMovingBigStone;
                }
    ...
    ISSRState StartFlee(Vector3 flee_from)
    {
        ...
        if (Vector3.Distance(oiLastLocation(Myself), flee_from) < 3)
        {
            ...
            flee_location = oiLocation(Myself) + direction.normalized * 1.5f;
